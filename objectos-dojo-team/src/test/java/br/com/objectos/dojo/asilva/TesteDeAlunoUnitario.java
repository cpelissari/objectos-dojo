 ...
 */
@Test
public class TesteDeAlunoUnitario {

  private AlunoReader reader;

  private Asserts asserts;

  @BeforeClass
  public void prepararReader() {
    TxtIteratorGen txtIteratorGen = new TxIteratorGenFalso();
    ToArrayString toArrayString = new ToArrayStringFalso();
    ToAluno toAluno = new ToAlunoFalso();
    reader = new AlunoReaderImpl(txtIteratorGen, toArrayString, toAluno);
  }
  ...
  public void deve_gerar_iterator_aluno() throws URISyntaxException {
    File file = TxtsFalso.getFile("/Aluno/Ler.txt");

    Iterator<Aluno> iterator = reader.of(file);
    ...
    assertThat(asserts.toAlunoOK, equalTo(true));
    ...
    assertThat(ordem.get(2), equalTo("ToAluno"));
  }
  ...
  private class TxIteratorGenFalso implements TxtIteratorGen {
    @Override
    public TxtIterator gerar(File file) {
  ...
  private class ToAlunoFalso implements ToAluno {
    @Override
    ...
    public Iterator<Aluno> transform(Iterator<String[]> colunas) {
      asserts.ordemDeMetodos.add("ToAluno");
      asserts.toAlunoOK = true;
      return Iterators.emptyIterator();
    }
  }
}

class TxtIteratorVazio implements TxtIterator {
